...
fun KVTApp(
    ...
    KakkamvellyTempleTheme {
        ...
        // Pager state — drives both swipe and bottom nav
        ...
        Scaffold(
                            ...
                            icon = {
                                if (tab == NavTab.Vazhipad)
                                    Text("ॐ", style = MaterialTheme.typography.titleLarge.copy(
                                        color = if (pagerState.currentPage == idx) Gold
            ...
            HorizontalPager(
                ...
                beyondViewportPageCount = 1  // keep neighbours alive
            ) { page ->
                when (TABS[page]) {
    ...
    TopAppBar(
        ...
        actions = {
            Row(
                ...
                LangChip("മലയാളം", selected = !isEn) { if (isEn) onToggleLang() }
                LangChip("EN",      selected = isEn)  { if (!isEn) onToggleLang() }
            }
